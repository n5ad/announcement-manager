...
if [[ ! -f "$CONVERT_SCRIPT" ]]; then
    ...
    # ----- audio_convert.sh (with variable leading pause) -----
    CONVERT_SCRIPT="$LOCAL_DIR/audio_convert.sh"
    echo "Creating/updating $CONVERT_SCRIPT with pause support"
    cat > "$CONVERT_SCRIPT" << 'EOF'
#!/bin/bash
#
...
EOF
    ...
    echo "Created/updated $CONVERT_SCRIPT with pause support"
