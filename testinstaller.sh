...
if [ ! -f "$INI_FILE" ]; then
    ...
    if grep -q "^\[${NODE_NUMBER}\]" "$INI_FILE" && grep -q "iframepost" "$INI_FILE"; then
        ...
        echo "Adding iframe lines under existing [${NODE_NUMBER}] section ..."

        # Insert inside the existing section (right after the [node] line)
        sed -i "/^\[${NODE_NUMBER}\]/a\\
iframepost=/announcement-manager/allmon-announcement-frame.php\\
iframepre=/supermon/usflag.jpg" "$INI_FILE"
...
# ----- audio_convert.sh (download from repo) -----
CONVERT_SCRIPT="$LOCAL_DIR/audio_convert.sh"

sudo wget -q -O "$CONVERT_SCRIPT" https://raw.githubusercontent.com/n5ad/announcement-manager/main/audio_convert.sh
...
if [ -f "$CONVERT_SCRIPT" ]; then
    ...
    echo "Downloaded and installed $CONVERT_SCRIPT successfully"
else
    echo "Failed to download audio_convert.sh — falling back to embedded version"
    # (optional fallback code if you want one)
fi
