...
# STEP 1. Install required packages FIRST â€“ force git install
...
if [[ -f "$SOURCE_INC" ]]; then
    ...
    warn "Source file $SOURCE_INC not found â€“ skipping Allmon3 copy (check repo contents)"
fi
...
#!/bin/bash
#
# playglobal.sh â€“ Play an audio file over an AllStarLink v3 node (Debian 12)
NODE="$NODE_NUMBER"
if [ "\$EUID" -ne 0 ]; then
...
EOF
    ...
    echo "$GLOBAL_SCRIPT already exists â€“ skipping"
fi
# ----- polite_global.sh -----
...
EOF
    ...
    echo "$POLITE_GLOBAL_SCRIPT already exists â€“ skipping"
fi
# ----- playaudio.sh -----
...
EOF
    ...
    echo "$PLAY_SCRIPT already exists â€“ skipping"
fi
# ----- polite_play.sh -----
...
EOF
    ...
    echo "$POLITE_PLAY_SCRIPT already exists â€“ skipping"
fi
# ----- audio_convert.sh -----
...
EOF
    ...
    echo "$CONVERT_SCRIPT already exists â€“ skipping"
fi
...
if [ ! -f "$LINK_PHP" ]; then
    echo "  â†’ link.php not found at $LINK_PHP â†’ skipping patch"
else
    ...
    if grep -q "include_once.*custom/announcement.inc" "$LINK_PHP"; then
        echo "Announcement include already present in link.php â€” skipping patch"
    else
...
# STEP 10.1 â€“ Apply your preferred IPv4 LAN detection block
...
if [[ -f "$INDEX_FILE" ]]; then
    if [[ ! -f "$INDEX_ORIG" ]]; then
        ...
        echo "Original backup $INDEX_ORIG already exists â€“ skipping backup"
    fi
fi
...
if [[ ! -f "$INDEX_SPARE" ]]; then
    ...
    echo "Spare copy $INDEX_SPARE already exists â€“ skipping"
fi
...
#    echo "$SUDOERS_FILE already exists â€“ skipping"
# else
...
if [[ "$ARCH" == "x86_64" || "$ARCH" == "amd64" ]]; then
    ...
    echo "Detected x86_64 architecture â€“ using amd64 binary"
elif [[ "$ARCH" == "aarch64" || "$ARCH" == "arm64" ]]; then
    PIPER_FILE="piper_arm64.tar.gz"
    echo "Detected aarch64/arm64 architecture â€“ using arm64 binary"
else
    echo "Error: Unsupported architecture: $ARCH"
    exit 1
fi
if [[ -f "/opt/piper/bin/piper" && -f "/opt/piper/voices/en_US-lessac-medium.onnx" ]]; then
    echo "Piper and voice model already installed â€“ skipping"
else
...
if [[ -f "/usr/local/bin/piper_prompt_tts.sh" ]]; then
    echo "/usr/local/bin/piper_prompt_tts.sh already exists â€“ skipping"
else
...
echo_step "16. Setup complete â€“ verification"
echo "I hope you get a lot of use from this"
echo "Log into Supermon or Allmon3 â†’ Announcements Manager should now appear at the bottom."
echo "73 â€” N5AD"
