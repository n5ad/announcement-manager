...
<!-- Begin HamClock Embed this complete section has been commented out so that it will not show up
<div style="text-align:center; margin-bottom: 10px;">
    <iframe src="http://44.27.129.114:8080/live.html" width="800" height="480" style="border:none;"></iframe>
</div>
 End HamClock Embed -->

   
<! Begin Java Script to implement all the announcement functions -->
...
$(document).ready(function () {
    ...
    // ====== UL Dropdown for Play Now ======

    function loadULDropdown() {

        $.getJSON("custom/list_ul.php", function(files) {

            $("#ul_select").empty().append($("<option>").val("").text("-- Select UL File --"));
        ...
        $.getJSON("custom/list_cron.php", function(crons) {
            ...
            crons.forEach(function(c) {
                ...
                let editBtn = $("<button>").text("Edit").click(function() {
                    ...
                    let min   = prompt("Minute (0-59 or *):", parts[0]); if(min===null) return;

                    let hour  = prompt("Hour (0-23 or *):", parts[1]); if(hour===null) return;

                    let dom   = prompt("Day of month (1-31 or *):", parts[2]); if(dom===null) return;

                    let month = prompt("Month (1-12 or *):", parts[3]); if(month===null) return;

                    let dow   = prompt("Day of week (0-7 or *):", parts[4]); if(dow===null) return;
    ...
    // ====== Install Announcement (Convert MP3 → UL + Cron) ======
    ...
    $("#install_announcement").click(function() {
        ...
        if(!confirm("Install announcement: " + mp3 + " ?")) return;


        let min   = prompt("Enter minute (0-59 or *):", "45"); if(min===null) return;

        let hour  = prompt("Enter hour (0-23 or *):", "8-21"); if(hour===null) return;

        let dom   = prompt("Enter day of month (1-31 or *):", "1-24"); if(dom===null) return;

        let month = prompt("Enter month (1-12 or *):", "*"); if(month===null) return;

        let dow   = prompt("Enter day of week (0-7 or *):", "*"); if(dow===null) return;

        let desc  = prompt("Enter description for this announcement:", "Scheduled announcement: "); 

        if(desc===null || desc.trim()==="") { alert("You must enter a description."); return; }
        ...
        $.post("custom/announcement.php", {
            ...
            loadULDropdown(); // reload UL dropdown after conversion

            loadCronList();   // reload cron list
    ...
    $("#run_ul").click(function() {
        ...
        if(!ulFile) { alert("Please select a UL file."); return; }

        if(!confirm("Play now: " + ulFile + "?")) return;
